...
class CCBaseModel(BaseModel):
    ...
    def uid(self) -> ModelUid:
        """A predictible uid (typically a hash) that represents the object."""

    def _hash_tokens(self, *tokens: str | int | "CCBaseModel") -> str:
        ...
        hash = hashlib.sha256()
        for token in tokens:
            token_str = token.uid if isinstance(token, CCBaseModel) else str(token)
