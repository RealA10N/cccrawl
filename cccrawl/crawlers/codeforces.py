...
class CodeforcesCrawler(Crawler):
    ...
    async def crawl(self, config: UserConfig) -> list[CrawledSubmission]:
        ...
        return [
            CrawledSubmission(
                problem_url=self._generate_problem_url(sub),
                verdict=(
    ...
    @staticmethod
    def _generate_problem_url(submission: dict[str, Any]) -> str:
        contest_id = int(submission["contestId"])
        problem_id = submission["index"]
